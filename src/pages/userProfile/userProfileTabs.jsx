  ...
  return (  
   <div className='w-[100%]'> 
    ...
    <div className="w-100% h-[100vh] fixed bg-[black] bb">
        {
                modalComment ?
                    <div className="w-[100%] h-[100vh] fixed left-0 top-0 flex flex-wrap justify-center items-center" style={{ backgroundColor: "rgba(0,0,0,0.5)" }}>
                        ...
                        <div className="w-[80%] h-[650px] rounded-[0px] mb-[20px] bg-[white] flex">
                            <div className="w-[53%] h-[100%] overflow-hidden flex justify-center items-center bg-[black]">
                                            ...
                                            objPost.comments.map((e) => {
                                                ...
                                                return (
                                                    <div className="w-[95%] sm:w-[100%] h-[60px] flex justify-between items-center text-[14px">
                                                        ...
                                                        <button onClick={() => deletecoment(e.postCommentId, e.userId == getToken().sid)} className='w-[24px] h-[24px] text-gray-500 flex justify-end items-center'>
                                                            <p>{e.userId == getToken().sid ? "delete" : "•••"}</p>
                                                        </button>
                                                    </div>
                                                )
                                            })
    ...
    <Box sx={{ width: '100%' }}>
      <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
        <Tabs sx={{zIndex:"1", position:"relative"}} value={value} onChange={handleChange} centered aria-label="basic tabs example">
          <Tab label={<p className='text-[12px]'>Публикации</p>} {...a11yProps(0)} />
          <Tab label={<p className='text-[12px]'>Reels</p>} {...a11yProps(1)} />
          <Tab label={<p className='text-[12px]'>Отметки</p>} {...a11yProps(2)} />
        </Tabs>
      </Box>
      <CustomTabPanel value={value} index={0}>
        {post.filter((e)=>e.userId == id).length !=0?
        <div className="w-[100%]  flex flex-wrap  gap-[0px]">
            { post.map((e)=>{
                if(e.userId == myid){
                    let thisUser = user?.find(element => element?.id == e?.userId)
                        return (
                        <div onClick={()=>openModalComment(e, e.postId, thisUser)} className="w-[32%] bg-[black] rounded-[5px] m-[6px] flex justify-center items-center overflow-hidden">
                            <img className=' h-[309px] zxc' src={`${import.meta.env.VITE_APP_FILES_URL}${e.images[0]}`} alt="" />        
                        </div>
                        )
                }
            })  
        ...
        <div className="w-[100%]  h-[350px] flex flex-wrap justify-center content-between">
      ...
      <CustomTabPanel value={value} index={1}>
           <div className='w-[100%] h-[350px] flex flex-wrap justify-center items-center gap-[0px]'>
                         
            {/* {
              saves.length == 0 ? <div className="div"><div className="w-[100%] flex justify-between">
          <div className="w-[350px] flex flex-wrap justify-center text-center">
              <div className="w-[63px] h-[63px] rounded-[50%] border-[2px] flex justify-center items-center">
                  <BookmarkBorderOutlinedIcon style={{fontSize:"40px"}}/>
              </div>
            ...
            return <div  onClick={()=>openModalComment(e,e.postId)} key={e.id} className='w-[32%] bg-[black] rounded-[5px] m-[6px] flex justify-center items-center overflow-hidden'>

               {
                 e.images.map((elem)=>{
                   return <img className='h-[309px] zxc' src={`${import.meta.env.VITE_APP_FILES_URL}${elem}`} alt="" />
                  })
                }
            </div>
          }) 
      ...
      <CustomTabPanel value={value} index={2}>
        <div className="w-[100%] h-[350px] flex flex-wrap justify-center content-center">
                <div className="w-[350px] flex flex-wrap justify-center text-center">
                    <div className="w-[63px] h-[63px] rounded-[50%] border-[2px] flex justify-center items-center">
